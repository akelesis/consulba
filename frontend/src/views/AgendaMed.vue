...
export default {
    ...
    methods: {
      ...
      cadastraAgenda(){
        alert(this.horasDisponiveis + "----" + this.flagHora)
        //this.appoint.date = this.actualDate
        //this.appoint.time =

        //alert(JSON.stringify(this.appoint))
        //axios.post('http://localhost:3000/appointment')
      },
      addToHorasDisponiveis(hora) {
        if(this.horasDisponiveis.length == 0){
          ...
          for(let i = 0; i < this.horasDisponiveis.length; i++){
            ...
            else if(this.horasDisponiveis[i] != hora && this.flagHora == false){
              this.horasDisponiveis.push(hora)
              this.flagHora = true
            }
          }
